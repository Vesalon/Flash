...
function PickerCtrl($scope, $mdDialog, currentDate, $mdMedia, $timeout) {
	var self = this;
    this.VIEW_HOURS = 1;
    this.VIEW_MINUTES = 2;
    this.currentDate = currentDate;
...
module.provider("$mdpPicker", function() {
    var LABEL_OK = "Save",
        LABEL_CANCEL = "Cancel"; 
    ...
    this.$get = ["$mdDialog", function($mdDialog) {
        var Picker = function(targetEvent, currentDate) {
            if(!angular.isDate(currentDate)) currentDate = Date.now();
    
            return $mdDialog.show({
                controller:  ['$scope', '$mdDialog', 'currentDate', '$mdMedia', '$timeout', PickerCtrl],
                controllerAs: 'picker',
                clickOutsideToClose: true,
				templateUrl: 'templates/private/haps/time-picker.html',
                targetEvent: targetEvent,
                locals: {
                    currentDate: currentDate,
